...
const Events = () => {
  ...
  const eventsData = [
    ...
    },
    {
      ...
      image: 'src/Assets/VIHAAN Registration Open.png',
      link: '',
    },
    {
  ...
  React.useEffect(()=>{
    if(eval(`${window.innerWidth}`)<655){
      cnum(1)
    }
    else if(eval(`${window.innerWidth}`)<1057){
      cnum(2)
    }
    // console.log(window.innerWidth);
  },[window.innerWidth])
  return (
    // <div id="events" className="row clients-outer  p-0 mt-6 w-[100" data-aos="fade-up">
        ...
        <Slider {...settings}>
          {eventsData.map((event, index) => (
            <div key={index} className="clients__slide flex space-x-4 p-4 ">
              <p>
                <div className="bg-[rgba(110,43,127,1)] p-4 flex-1 transition-transform transform hover:scale-105 h-[630px] eventbox2 ">
                  <img
                    src={event.image}
                    alt={event.title}
                    className="w-64 h-64 mx-auto event-img"
                  />
